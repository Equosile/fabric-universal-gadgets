...
public class OFAAFO_Dice_Pentapip extends ExperienceDroppingBlock {

    public static final int pentapip_MAX_ENERGY = 5;
    public static final IntProperty pentapipEnergy = IntProperty.of("pentapip_energy", 0, pentapip_MAX_ENERGY);
    public OFAAFO_Dice_Pentapip(Settings settings, IntProvider experience) {
        super(settings, experience);
        this.setDefaultState(this.getDefaultState().with(pentapipEnergy, pentapip_MAX_ENERGY));
    }

    @Override
    protected void appendProperties(StateManager.Builder<Block, BlockState> builder) {
        builder.add(pentapipEnergy);
    }

    @Override
    public void appendTooltip(ItemStack stack, @Nullable BlockView world, List<Text> tooltip, TooltipContext options) {
        if (Screen.hasShiftDown()) {
            tooltip.add(Text.translatable("tooltip.ofaafo.dice_pentapip.tooltip.shift"));
        } else {
            tooltip.add(Text.translatable("tooltip.ofaafo.dice_pentapip.tooltip"));
        }
    }
        ...
        if (!world.isClient()) {
            ...
            if (hand == Hand.MAIN_HAND) {
                if (player.isSneaking()) {
                    ...
                    pentapip_Emit_Redstone(state, world, pos);
                }
            }
        }
    ...
    public int getStrongRedstonePower(BlockState state, BlockView world, BlockPos pos, Direction direction) {
        ...
        return pentapipCheckState(state) ? pentapip_MAX_ENERGY : 0;
    }
    ...
    public static boolean pentapipCheckState(BlockState state) {
